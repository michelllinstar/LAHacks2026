...
def build(
    ...
    *,
    ...
    if not func_ids:
        logger.info("layer 2: no function/method symbols found; skipping flow build")
    ...
    if not roots:
        ...
        if out_degree:
            ...
            logger.warning(
                "layer 2: no clean call-graph roots (cycle); using %d high out-degree fallbacks",
                len(roots),
            )
        else:
            logger.info("layer 2: no call edges among function symbols; skipping")
            return {"flows": 0, "sensitive": 0, "capped": False}
    ...
    if len(collected) > max_flows:
        capped = True
        logger.warning(
            "layer 2: collected %d flows, capping to %d (sensitivity-first, then deeper, then lower out-degree)",
            len(collected),
            max_flows,
        )
